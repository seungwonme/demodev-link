...
const Threads: React.FC<ThreadsProps> = ({
  ...
  useEffect(() => {
    ...
    if (prefersReducedMotion) {
      // Render a simple gradient instead
      container.style.background = `linear-gradient(135deg, rgba(${color[0] * 255}, ${color[1] * 255}, ${color[2] * 255}, 0.1) 0%, transparent 100%)`;
      return;
    }
    ...
    if (!gl) {
      // Fallback to gradient if WebGL is not supported
      container.style.background = `linear-gradient(135deg, rgba(${color[0] * 255}, ${color[1] * 255}, ${color[2] * 255}, 0.1) 0%, transparent 100%)`;
      return;
    }
    ...
    return () => {
      ...
      if (container.contains(rendererGl.canvas)) container.removeChild(rendererGl.canvas);
